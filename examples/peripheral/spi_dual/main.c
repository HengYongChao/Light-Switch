...
/**@file
 * @defgroup spi_dual main.c
 * @{
 * @ingroup spi_master_example
 *
 * @brief Connect a MCP4822 dual-channel DAC chip to the BLE nano to use this code.
 */
...
static volatile bool dac = false; // set to DAC 0 at first
                         ...
                         const uint16_t  len)
{
    ...
    uint16_t i;
    uint16_t data,data2 = 0;
    //uint16_t data = negate ? 4095-(sineWave[len]) : sineWave[len]; // Value 0-4095
    switch(dac) {
	case false: // triangle wave
	    data = triangleWave[len];
	    data2 = (0x9000 | (0x0FFF & data)) >> 8;
	    break;
	case true: // reverse triangle wave
	    data = rTriangleWave[len];
	    data2 = (0x1000 | (0x0FFF & data)) >> 8;
	    break;
    }    
