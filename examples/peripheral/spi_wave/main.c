...
static void init_buffers(uint8_t * const p_tx_data, uint8_t * const p_rx_data, uint8_t wave, const uint16_t  len)
{
    ...
    switch(wave) {
	case 0: // sine wave
	    data = negate ? 4095-(sineWave[len]) : sineWave[len]; // Value 0-4095
	    break;
	case 1: // triangle wave
	    data = triangleWave[len];
	    break;
	case 2: // reverse triangle wave
...
void sendData(uint8_t wave, uint16_t index){
    init_buffers(m_tx_data, m_rx_data, wave, index);
